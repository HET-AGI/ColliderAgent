...
- Generator-level invariant mass cut: $m_{\ell\ell} > 0.9 \times M_{Z'}$ (to select events near the Z' resonance)
  - comment: this cut is not from the paper.
...
- comment: (massless limit, valid for $M_{Z'} \gg m_f$), is adopted
...
- comment: bosonic decay channels ($Z' \to W^+W^-$, $Z' \to ZH_{1,2}$) are neglected. The $WW$ channel is $\propto \sin^2\theta' \sim 10^{-6}$. The $ZH$ channels have a leading term $\propto \tilde{g}^2$ (not $\sin^2\theta'$), but are numerically suppressed by the prefactor $1/(768\pi)$ to $\lesssim 0.2\%$ of the total width.
...
- comment: since a $M_{Z'}$ dependent cut is used, the background event counts is a function of $M_{Z'}$.
- comment: the factor $\epsilon_{\text{acc}}$ depends on $m_{\ell\ell}$ actually. Here, an approximation is used due to the $M_{Z'}$ dependent cut.
...
- $\epsilon_{\text{acc}} = 0.6$: combined acceptance $\times$ efficiency for dilepton selection
  - comment: the paper does not explicitly give this value, so we take a typical value for the LHC Run 1.
