...
## Workflow in the Collider Agent Pipeline

The MadAnalysis step sits after MadGraph5 event generation:

1. **Read event_index.yaml** to locate event files for the target process
2. **Build MA5 script** with import, configuration, plots, and selections
3. **Execute via `connect_to_madanalysis()`** to produce analysis output

```python
# Step 1: Get event file paths
index = read_event_index("event_index.yaml", "pp_to_tt_3l")
event_files = index["event_files"]

# Step 2: Build and run MA5 analysis
script = f"""
import {event_files[0]} as ttbar
...
## Using connect_to_madanalysis()

`connect_to_madanalysis(script, level, timeout)` executes MadAnalysis5 commands.

**Input**: Multi-line string of MA5 commands OR path to .ma5 script file
**Output**: dict with success, stdout, stderr, return_code, message
...
### Example - Reco-Level Analysis

```python
script = """
import /path/to/signal.lhco.gz as sig
import /path/to/background.lhco.gz as bkg
set sig.type = signal
...
select N(mu) >= 2
select PT(mu[1]) > 25
submit analysis_output
"""
result = connect_to_madanalysis(script, level="reco")
```

### Example - Run Script File

```python
result = connect_to_madanalysis("scripts/my_analysis.ma5", level="reco")
```

## Using read_event_index()

`read_event_index(yaml_path, process_name, event_set_name)` parses the event index.

**Input**: Path to event_index.yaml, process name, energy label
**Output**: dict with success, process_name, base_path, runs, event_files, message
...
### Example 4: Multi-Sample Analysis from Event Index

This shows the typical workflow using `read_event_index()` to build scripts programmatically:

```python
# Read event locations
index = read_event_index("event_index.yaml", "pp_to_tt_3l")
bkg_files = index["event_files"]

index_sig = read_event_index("event_index.yaml", "pp_to_tS_3l_decay")
sig_files = index_sig["event_files"]
sig_runs = index_sig["runs"]

# Build MA5 script
lines = []

# Import background (combine all runs)
for f in bkg_files:
    lines.append(f"import {f} as bkg_tt")
lines.append('set bkg_tt.type = background')

# Import signal (one mass point)
# sig_runs[4] corresponds to mH0=100
lines.append(f"import {sig_files[4]} as sig_100")
lines.append('set sig_100.type = signal')

# Configure
lines.append('set main.lumi = 100')
lines.append('set main.normalize = lumi')

# Plots and selections
lines.append('plot MET 50 0 500')
lines.append('plot M(mu+ mu-) 50 0 200')
lines.append('select N(mu) >= 2')
lines.append('select PT(mu[1]) > 25')

lines.append('submit analysis_3l_mH100')
...
3. Increase timeout for large samples: `connect_to_madanalysis(script, timeout=3600)`
