...
| A′ | Claude Opus 5 | 同上 | 本轮补跑 5 题 ×1（ALP、U(1)′ 为 9/16 数据）；MuC reach 用了 4.3 h：主 agent 发现低质量端被统计噪声支配后派子 agent 补高统计样本再重画 |
...
| Scalar LQ（2005.06475 Fig. 2，重） | 0/1（TBD） | – | 0/1 | 1/1 | 0/1 | – | 0/1 |
...
3. **长程重题是分水岭**：Scalar LQ / mono‑τ 需要 10 万事例的 Pythia8+Delphes、轻子→光子的 LHE 改写、10 GB 作业空间限制下的分批与清理，20–40 个作业、2.5–6 h。编码型 agent 用自写脚本 + 重新 launch 解决（Codex gpt‑5.5 4.4 h 完成 Scalar LQ，gpt‑5.3‑codex 2.6 h 完成 mono‑τ 且排除限最接近论文）；ADK+gpt‑5.5 在 Scalar LQ 上遇到"工具不支持 LHE 改写"、在 mono‑τ 上遇到"Delphes 输出下载不了"都退回 parton 级近似（两题均判失败）；ADK+Gemini 的 mono‑τ 在 300 次调用内连编译都没通过，Scalar LQ 在 300 次内 launch 失败 13 次后触顶。重题合计：编码型 agent 4/6，ADK 0/4。
...
| ADK Gemini Scalar LQ | 失败（generation）：上限 300 次耗尽，14 次 launch 失败 13 次，81 个报错‑修正循环，上下文 199k，2.8 h |
