...
- **U(1)′ 3 TeV 面板**：Codex/gpt‑5.5 复现出论文的轮廓尖端（g̃ ≈ −0.72，g₁′ ≈ 0.65），而 Opus 4.6 三次和 Opus 5 一次都给出 0.48。因此 2026‑09‑17 文档里"该偏差来自 prompt 的分析流程而非 agent"的说法撤回：prompt 足以得到论文结果。**9/26 再更正**：gpt‑5.3‑codex 与 ADK+gpt‑5.5 也给出 0.48、Gemini CLI 给 0.55，0.65 只出现在 Codex gpt‑5.5 一次——这是多数 run 共有的分析选择，不是 Claude 特有（见 `results-2026-09-26/README.md` 第 4 节）。
