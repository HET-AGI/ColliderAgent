...
After copying, use `rg` to check for any remaining absolute paths in the copied scripts:

```
rg -n '/Users/|/home/|/tmp/' reproduction/scripts/
```

- **If no matches** → all scripts are portable, proceed to Step 3.
- **If matches are found** → patch only those lines with relative-path equivalents (e.g., using `os.path.join(SCRIPT_DIR, ...)`). Do not load and rewrite the full file when the search output already provides enough context.

**Files to create** — only files that don't exist in the pipeline:
...
2. **cp-first strategy** — `cp` all existing pipeline scripts and the UFO directory first. Then use `rg` to check for absolute paths; if any are found, patch only the matching lines. Create only files that don't exist in the pipeline (`generate_ufo.wl`, `run_all.sh`, `README.md`). Never recreate an existing file from scratch.
