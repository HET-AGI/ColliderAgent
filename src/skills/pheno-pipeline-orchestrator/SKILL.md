...
On the Codex adapter branch the stage subagents are the project-scoped custom agents in `.codex/agents/` with the same names; spawn only the next required stage, wait for it, read its sidecar, then build the next stage's prompt. Do not run dependent or write-heavy stages concurrently.
